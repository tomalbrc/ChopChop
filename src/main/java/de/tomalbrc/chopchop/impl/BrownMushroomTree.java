...
public class BrownMushroomTree implements TreeType {
    ...
    public BrownMushroomTree(TreeConfig config) {
        this.config = config;
    }
	...
	public TreeData gatherTreeData(BlockPos blockPos, ServerLevel level, Player player) {
		...
		stemBlocks.forEach((key, value) -> capBlocks.putAll(gatherCapBlocks(level, key.above())));
		...
		if (level instanceof ServerLevel serverLevel) {
			for (Map.Entry<BlockPos, TreeData.BlockInfo> entry : blocks.entrySet()) {
				BlockState blockState = entry.getValue().blockState();

				List<ItemStack> items = Block.getDrops(blockState, serverLevel, entry.getKey(), null, player, player.getMainHandItem());
